...
const KeyboardFocusSection: React.FC<SectionProps> = ({ id }) => {
  ...
  const [tabElements, setTabElements] = useState<{top: number, left: number}[]>([]);
  const containerRef = useRef<HTMLDivElement>(null);

  useLayoutEffect(() => {
    if (showTabOrder && containerRef.current) {
        const focusable = Array.from(
            containerRef.current.querySelectorAll<HTMLElement>('a, button, input, [tabindex]:not([tabindex="-1"])')
        ).filter(el => el.offsetWidth > 0 && el.offsetHeight > 0); // Only visible elements

        const positions = focusable.map(el => ({
            top: el.offsetTop,
            left: el.offsetLeft,
        }));
        setTabElements(positions);
    } else {
        setTabElements([]);
    }
  }, [showTabOrder]);
    ...
    <SectionCard title="Điều hướng bằng Bàn phím & Quản lý Tiêu điểm (Focus)" id={id}>
      ...
      <div className="mt-4 p-4 border border-slate-200 dark:border-slate-700 rounded-lg">
        <div className="flex justify-between items-center mb-4">
            <h3 className="font-semibold text-lg">Thử điều hướng bằng phím Tab</h3>
            <button
                onClick={() => setShowTabOrder(!showTabOrder)}
                className="px-3 py-1.5 text-sm bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 rounded-md font-medium"
                aria-pressed={showTabOrder}
            >
                {showTabOrder ? 'Ẩn' : 'Hiển thị'} thứ tự Tab
            </button>
        </div>
        <p className="mb-4">
        ...
        <div ref={containerRef} className="relative flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-4 sm:space-y-0 p-2">
          <a 
            href="#" 
          ...
          />
          {showTabOrder && tabElements.map((pos, index) => (
            <div
                key={index}
                className="absolute flex items-center justify-center w-6 h-6 bg-indigo-600 text-white text-xs font-bold rounded-full ring-2 ring-white dark:ring-slate-800"
